...
def extract_mutual_friends(profiles_urls):
	...
	for profile_url in profiles_urls:
		...
		xpath_name_params3 = ".//div[@class='fsl fwb fcb']/a/@data-hovercard"
		...
		for prm in zip(params_result3):
		    print prm
		    prm = str(prm)
		    sep = '&extragetparams'
		    rest1 = prm.split(sep, 1)[0]
		    sep2 = '/ajax/hovercard/user.php?id='
		    rest2 = rest1.split(sep2, 1)[1]
		    user.add_fbfriend(rest2)
		    if rest2 is None:
		        rest2 = ""
		    print rest2
		    if rest2 not in data:
		        data.append(rest2)
		  #  result = re.search('(?<=\user\.php\?id=)(.*\n?)(&extragetparams)', prm)
...
# 						accessT = '613995812038257|23MVoIyzLtyTHQLXifksC-PrBGY'
# 						reqString = "https://graph.facebook.com/"+user+"?fields=name&access_token=" + str(accessT)
# 						#reqString = "https://graph.facebook.com/"+userID+"?fields=email&access_token=" + str(access_token)
# 						got = requests.get(reqString)
# 						#email = got.json()["id"]
# 						#myFile.write("id: " + email+"\n")
...
target = args.target
user = models.User.query.filter_by(fb_id = int(target)).first()
rijul = models.User.query.filter_by(nickname = "Rijul Gupta").first()
profilesfile = rijul.fbfriends.split(', ')
if user.fbfriends is not None:
    profilesfile = user.fbfriends.split(', ')
...
# all_cookies = dict()
def get_the_user(get_cookies):
    url = 'http://facebook.com/%s' % args.target
    all_cookies = dict()
    for cookie in cookies:
    	all_cookies[cookie["name"]]=cookie["value"]
    req = requests.get(url, cookies=all_cookies)
    finalUrl = str(req.url)
    finalName = finalUrl.replace('https://www.facebook.com/','')
    global target
    target = finalName

get_the_user(cookies)
...
# formdata = { "username" : 'rijul.gupta@gmail.com', "password": 'Rijul191992'}
...
# for cookie in cookies:
# 	all_cookies[cookie["name"]]=cookie["value"]
# starturl = 'http://facebook.com/%s' % args.target
# #req = urllib2.Request(starturl)
# req = urllib2.get(starturl, cookies=cookies)
# res = urllib2.urlopen(req)
# finalurl = str(res.geturl())
# finalName = finalurl.replace('https://www.facebook.com/','')
# target = finalName

# daURL = "https://www.facebook.com/profile.php?id=%s" % args.target
# daReq = requests.get(daURL, cookies=cookies)
...
if args.target:
		profiles = profilesfile#open_file(profilesfile)
